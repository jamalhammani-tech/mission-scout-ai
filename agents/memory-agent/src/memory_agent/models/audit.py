...
class AuditEventModel(Base, UUIDPk):
    ...
    acteur_type: Mapped[ActeurType] = mapped_column(
        SAEnum(ActeurType, native_enum=False, length=20), nullable=False
    )
    acteur_identifiant: Mapped[str] = mapped_column(String(150), nullable=False)

    proprietaire_user_id: Mapped[uuid.UUID | None] = mapped_column(
